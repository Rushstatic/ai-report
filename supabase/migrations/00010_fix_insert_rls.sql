...
-- SUB CENTRES POLICIES (Taluka Controller)
CREATE POLICY "Taluka controller update own subcentres" ON public.sub_centres FOR UPDATE TO authenticated
USING (
  EXISTS (SELECT 1 FROM public.employees e JOIN public.phcs p ON p.taluka_id = e.taluka_id WHERE e.user_id = auth.uid() AND e.employee_type = 'TALUKA_CONTROLLER' AND public.sub_centres.phc_id = p.id AND e.status = true)
);
...
-- SUB CENTRES POLICIES (PHC Controller)
