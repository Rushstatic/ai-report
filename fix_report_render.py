...
# Add buildTree function and renderFieldNode
new_code = r"""
  // ----------------------------------------------------
...
# Replace form.fields.map((field) => ( ... )) with buildFieldTree
import re
content = re.sub(r"form\.fields\.map\(\(field\) => \(.*?\)\)[\s\S]*?(?=\s*\)\s*\}[\s\S]*?<\/div>[\s\S]*?\{/\* Action Buttons \*/\})", "buildFieldTree(form.fields).map((field) => renderReportFieldNode(field))\n          ", content, flags=re.DOTALL)
